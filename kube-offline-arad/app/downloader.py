...
def _normalize_registry_project(project: str | None) -> str:
    if project is None:
        return ""
...
def _registry_dest(
    ...
    """Build destination ref; inject Harbor project for external registry hosts.

    Examples:
      host=hub.example.com, project=local, ref=docker.io/mirantis/img:1
        → hub.example.com/local/mirantis/img:1
      host=hub.example.com/local (project already in path)
        → hub.example.com/local/mirantis/img:1
      host=None (local Compose registry) — no project prefix
        → registry:5000/mirantis/img:1
    """
    base = _normalize_registry_host(host)
    path = _image_path(ref)
    # User already included a project path in the registry address.
    if "/" in base:
        return f"{base}/{path}"
    # Local default registry keeps flat paths (Docker Registry v2, not Harbor).
    if not host or not str(host).strip():
        return f"{base}/{path}"
    proj = _normalize_registry_project(
        project if project is not None else settings.registry_project
    )
    if proj:
        return f"{base}/{proj}/{path}"
    return f"{base}/{path}"
...
def pull_and_push_image(
    ...
    *,
    ...
    if settings.push_to_registry or registry_host:
        dest_ref = _registry_dest(ref, registry_host, registry_project)
...
def push_saved_images(
    *,
    ...
    # Effective Harbor-style target host[/project] for logs.
    base = _normalize_registry_host(registry_host)
    if registry_host and "/" not in base:
        proj = _normalize_registry_project(
            registry_project
            if registry_project is not None
            else settings.registry_project
        )
        target = f"{base}/{proj}" if proj else base
    else:
        target = base
    job.set_phase(
        "push",
